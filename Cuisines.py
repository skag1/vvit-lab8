...
class Cuisines(QWidget):
    ...
    def _update_teacher_table(self):
        ...
        self.cursor.execute("SELECT full_name, subject_name, id\
    ...
    def _change_teacher(self, rowNum, id):
        ...
        try:
            if row.count('') == self.teacher_table.columnCount() - 1:
                ...
                self.cursor.execute(f"UPDATE teacher SET full_name='{row[0]}', subject_name='{row[1]}'\
                                    WHERE id={id}")
            self.conn.commit()
    ...
    def _add_record(self, row):
        ...
        try:
            self.cursor.execute(f"INSERT INTO teacher (full_name, subject_name)\
