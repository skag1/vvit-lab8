...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self._connect_to_db()
        self.setWindowTitle("mtuci_schedule2")

        self.vbox = QVBoxLayout(self)
        self.tabs = QTabWidget(self)
        self.vbox.addWidget(self.tabs)

        self.schedule_tab = Restaurants(self.conn)
        self.tabs.addTab(self.schedule_tab, 'Расписание')

        self.teacher_tab = Cuisines(self.conn)
    ...
    def _connect_to_db(self):
        self.conn = psycopg2.connect(database="mtuci_schedule3",
                                     user="postgres",
                                     password="123",
                                     host="localhost",
                                     port="5432")
